...
class App extends Component {
  ...
  async componentDidUpdate (prevProps, prevState) {
    if ((prevState.searchInput !== this.state.searchInput) || (prevState.page !== this.state.page)) {
      try {
        ...
        const fetchedSearch = await fetchData(this.state.searchInput, this.state.page)        
  ...
  render() {  
    const { loading,error,images, showBtn, showModal, modalImg, modalAlt } = this.state;
    return (
      <div className="App">
        <Searchbar onSubmit={this.formSubmitHandler} />
        {loading && <Loader/>}
        {error && <div>{error}</div>} 
        {images.length > 1 && <ImageGallery searchedArray={images} imgClick={this.handleImgClick} />}
        {showBtn && <Button loadMore={() => this.handleLoadMore()} />}
        {showModal && (<Modal onClose={this.toggleModal} modalSrc={modalImg} modalAlt = {modalAlt} />)} 
      </div>
    );
  }
};
